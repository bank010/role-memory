...
│  routes/api.py          纯 HTTP 路由（APIRouter）            │
│  serve/chat.py          对话服务（调 MemoryBox + LLM）        │
├─────────────────────────────────────────────────────────────┤
│  core/                  记忆引擎（MemoryBox，自包含可复用）     │
│                                                              │
│  ┌──────── 读路径（在线，毫秒级）────────────────────────┐   │
│  │  1. MemoryBox.build_prompt                           │   │
│  │     ├─ L0 人设（前端直传 persona_text）               │   │
│  │     ├─ L1 facts（画像）+ relationship（关系/摘要）    │   │
│  │     ├─ L2 情节向量召回 → Reranker 精排               │   │
│  │     └─ L3 逐字向量+关键词混合召回                     │   │
│  │  2. serve/chat → llm.chat(messages) → reply          │   │
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
│  ┌──────── 写路径（离线，异步不阻塞）──────────────────┐   │
│  │  3. MemoryBox.save_turn → turns 表落库               │   │
│  │  4. 后台异步：                                        │   │
│  │     ├─ chunks 逐字向量化（每轮）                      │   │
...
**三层分离**：
- **`core/`** — 记忆引擎，纯相对 import，可独立复用到任何项目
- **`serve/`** — 业务服务层，调 MemoryBox 拿 prompt，调 LLM 拿回复
- **`routes/`** — 纯 HTTP 路由，用 `APIRouter`，只做参数转发
...
Reranker 使用 Qwen3 官方 chat 模板（`core/client/rerank.py` 已封装），裸文本调用会得到随机分数。
...
抽取 LLM 的输出由 `core/memory/schema.py` 中 44 个字段引导，key 格式：
...
系统的向量检索层采用**仓储模式**（`core/store/base.py` 抽象接口），可按需切换存储后端，无需改业务代码。
...
| 数据库 | 推荐理由 | 适合规模 | 接入方式 |
|---|---|---|---|
| **pgvector**（当前集成）| 与 PostgreSQL 共存，无额外运维，支持 HNSW/IVFFlat，事务内一致 | 百万向量以内 | `core/store/postgres.py` 已实现 |
| **Qdrant** | 纯向量数据库，过滤检索强（支持 `user_id` payload 过滤），Rust 写性能好，有 Cloud 托管 | 千万级，多租户 | 扩展 `core/store/base.py` 实现 |
| **Weaviate** | 自带多租户隔离（对应 user×role 模型），支持混合检索（BM25+向量），有 GraphQL 查询 | 千万级，SaaS 场景 | 扩展 `core/store/base.py` 实现 |
| **Milvus / Zilliz** | 高并发写入性能极强，分布式架构，适合全量用户画像向量化 | 亿级，高并发写 | 扩展 `core/store/base.py` 实现 |
...
**Redis 热缓存**（`core/util/cache.py`）：`facts` / `relationship` 两条主链路热读，读穿透 + 写失效 + TTL 兜底。未配置或连接失败时自动降级为直查后端，绝不成为故障点。
...
> **注意**：Qwen3-Reranker 系列必须通过官方 chat 模板调用（以 yes/no token 概率作为分数），裸文本调用会得到随机结果。本项目的 `core/client/rerank.py` 已正确封装此逻辑，Reranker 不可用时自动回退到纯向量三维打分，不影响功能。
...
# ── MongoDB 对话归档（留空=关闭，自动降级 no-op，不影响对话）──────
# 与核心记忆解耦的"对话流水 + 训练数据湖"：每轮异步落一份完整 prompt（含注入记忆）
# + 回复 + 元信息，供偏好训练（点赞/点踩/重新生成）和外部系统集成（GET /api/conversations）
# MONGO_URL=mongodb://localhost:27017
MONGO_DB=role_memory               # 数据库名
MONGO_ARCHIVE_COLLECTION=conversations  # 归档集合名
MONGO_MAX_POOL=100                 # 连接池上限
MONGO_TIMEOUT_MS=3000              # 连接/选主超时（毫秒）：连不上快速降级，不拖慢回复
...
```bash
uvicorn main:app --workers 4 --port 8011
```

### 压力测试

项目自带压力测试脚本（`test/stress_test.py`），支持自定义并发数和轮数：

```bash
python test/stress_test.py -c 200           # 200 并发，每用户 1 轮
python test/stress_test.py -c 1000 -r 3     # 1000 并发，每用户连续 3 轮
```
...
| 方法 | 端点 | 说明 |
|---|---|---|
| `POST` | `/api/chat` | 发送消息，获取 AI 回复（后台异步触发记忆加工 + 对话归档） |
| `GET` | `/api/memory` | 查询用户画像 / 情节库 / 关系状态 |
| `GET` | `/api/history` | 获取最近 N 轮对话原文（读 SQLite/PG 的 turns 表，刷新恢复用） |
| `GET` | `/api/conversations` | 从 MongoDB 归档库分页拉取历史对话（训练数据 / 外部集成） |
| `POST` | `/api/reprocess` | 重置加工进度，从头重新抽取所有记忆 |
| `POST` | `/api/reset` | 清空该会话全部记忆（不可逆） |
| `GET` | `/api/health` | 健康检查，返回各后端/开关状态 |

### `GET /api/conversations` — 历史对话归档查询

从 MongoDB 归档库读取完整对话记录。与 `/api/history` 的区别：

| | `/api/history` | `/api/conversations` |
|---|---|---|
| 数据源 | SQLite/PG 的 `turns` 表（在线记忆） | **MongoDB 归档库** |
| 用途 | 刷新页面恢复最近对话 | 偏好训练数据采集 / 外部系统集成 |
| 内容 | 仅对话原文 | 完整 prompt、注入的记忆、模型、耗时、反馈字段 |
| 分页 | 仅取最近 N 轮 | `skip` / `limit` 分页 + 排序 |

> 依赖 `MONGO_URL`（见环境变量）。未配置 / Mongo 不可用时返回空集 `{"total":0,"items":[],"enabled":false}`，不报错。

**查询参数**：

| 参数 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `user_id` | string | — | 按用户过滤（留空=不限） |
| `role_id` | string | — | 按角色过滤（留空=不限） |
| `limit` | int | 20 | 每页条数（上限 200） |
| `skip` | int | 0 | 偏移量（分页用） |
| `full` | bool | false | `false`=轻量列表（剔除 `messages`/`system_prompt`）；`true`=完整训练样本 |
| `order` | string | desc | `desc`=最新在前；`asc`=按时间正序 |

**请求示例**：

```bash
# 轻量列表（分页）
curl 'http://localhost:8011/api/conversations?user_id=u1&role_id=cat&limit=20&skip=0'

# 完整训练样本（含发给 LLM 的完整 messages 和注入的 system_prompt）
curl 'http://localhost:8011/api/conversations?user_id=u1&role_id=cat&full=true'
```
...
```json
{
  ...
  "items": [
    {
      "id": "6a2a293b7f02fd63ea634dae",   // 文档 id（供后续反馈接口引用）
      "user_id": "u1",
      "role_id": "cat",
      "turn": 12,
      "user_msg": "用户这轮说的话",
      ...
      "char_name": "Cat",
      "model": "deepseek-chat",
      "timing_ms": { "context": 80.2, "llm": 1320.5, "total": 1400.7 },
      "created_at": "2026-06-11T03:19:23.796000",
      "feedback": null,                    // 预留：点赞/点踩接口写入 up/down
      "regenerations": []                  // 预留：重新生成的候选回复
      // full=true 时额外返回：
      // "system_prompt": "...",           // 当时注入了哪些记忆（训练复现的关键）
      // "messages": [ ... ]               // 实际发给 LLM 的完整输入
    }
  ]
}
```

> 归档文档保留了**可复现训练样本**的全部信息（完整 prompt + 注入的记忆 + 回复），`feedback` / `regenerations` 字段为后续偏好训练（点赞/点踩/重新生成）预留。
...
```
role-memory/
├── main.py                  # FastAPI 入口（生命周期管理 + 挂载路由）
├── schemas.py               # Pydantic 请求体模型
├── personas.py              # L0 人设（兜底，角色由调用方通过 persona_text 管理）
│
├── core/                    # 记忆引擎（MemoryBox，自包含可复用）
│   ├── __init__.py          #   exports MemoryBox
│   ├── box.py               #   MemoryBox 对外接口（build_prompt / save_turn / get_memory）
...
│   │
│   ├── memory/              #   记忆处理（功能层）
│   │   ├── assembler.py     #     上下文拼装（L0~L3 + 占位符填充 + 记忆规则）
│   │   ├── retrieval.py     #     三维打分 + 逐字混合检索 + Reranker 精排
│   │   ├── pipeline.py      #     异步记忆加工管线（抽取/归纳/摘要/反思 + session 锁）
│   │   ├── stores.py        #     记忆编排层（向量化/去重/语义合并/缓存/淘汰）
│   │   └── schema.py        #     14 模块画像 Schema（多值/敏感/单值白名单定义）
│   │
│   ├── store/               #   存储后端（可插拔）
│   │   ├── base.py          #     抽象接口（ABC）
│   │   ├── sqlite.py        #     SQLite 实现（默认，零依赖，WAL 模式）
│   │   └── postgres.py      #     PostgreSQL + pgvector + HNSW（生产级）
│   │
│   ├── archive/             #   对话归档（与核心记忆解耦的数据湖）
│   │   └── mongo.py         #     MongoDB 异步归档（训练数据采集 / 外部集成，优雅降级）
│   │
│   └── util/                #   辅助工具
...
│
├── routes/                  # API 路由（纯 HTTP 接口，APIRouter）
│   └── api.py               #   chat / memory / history / conversations / reset / reprocess / health
│
├── serve/                   # 服务层（业务逻辑）
│   └── chat.py              #   LLM 对话（调 MemoryBox 拿 prompt → 调 LLM → 存回 MemoryBox）
│
├── static/                  # 前端
