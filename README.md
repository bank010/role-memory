...
Reranker 使用 Qwen3 官方 chat 模板（`core/rerank.py` 已封装），裸文本调用会得到随机分数。
...
抽取 LLM 的输出由 `core/profile_schema.py` 中 44 个字段引导，key 格式：
...
系统的向量检索层采用**仓储模式**（`core/store/base.py` 接口），可按需切换存储后端，无需改业务代码。
...
| 数据库 | 推荐理由 | 适合规模 | 接入方式 |
|---|---|---|---|
| **pgvector**（当前集成）| 与 PostgreSQL 共存，无额外运维，支持 HNSW/IVFFlat，事务内一致 | 百万向量以内 | `core/store/postgres_store.py` 已实现 |
...
**Redis 热缓存**（`core/cache.py`）：`facts` / `relationship` 两条主链路热读，读穿透 + 写失效 + TTL 兜底。未配置或连接失败时自动降级为直查后端，绝不成为故障点。
...
> **注意**：Qwen3-Reranker 系列必须通过官方 chat 模板调用（以 yes/no token 概率作为分数），裸文本调用会得到随机结果。本项目的 `core/rerank.py` 已正确封装此逻辑，Reranker 不可用时自动回退到纯向量三维打分，不影响功能。
...
│   └── store/               #   存储后端（可插拔）
│       ├── base.py          #     抽象接口（ABC）
│       ├── sqlite_store.py  #     SQLite 实现（默认，零依赖，WAL 模式）
│       └── postgres_store.py#     PostgreSQL + pgvector + HNSW（生产级）
│
